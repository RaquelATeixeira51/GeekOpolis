...
const cartUtils = {
    ...
    adicionaProdutoAoCarrinho: (produto) => {
        const cart = JSON.parse(localStorage.getItem('carrinho')) || cartUtils.initializeCart();
        const { produtos } = cart;
      
        const produtoExiste = produtos.find((item) => item.produto.id === produto.produto.id);
    ...
    removerProdutoDoCarrinho: (produto) => {
      ...
      calcularEAtualizarTotal: () => {
        ...
        produtos.forEach(produto => {
            const totalProduto = produto.produto.preco * produto.quantidade;
            total += totalProduto;
        });
